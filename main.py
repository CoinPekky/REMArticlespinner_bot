...
# Environment Variables (Securely pulled from Render's Environment tab)
TOKEN = os.getenv("TELEGRAM_BOT_TOKEN")
WEBHOOK_URL = os.getenv("RENDER_EXTERNAL_URL")  # Provided automatically by Render
GEMINI_API_KEY = os.getenv("GEMINI_API_KEY")

# Initialize OpenAI client pointing to Google Gemini's open-compatibility gateway
ai_client = OpenAI(
...
async def handle_tone_selection(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Triggers Gemini API to spin the text based on selected style."""
    ...
    await query.edit_message_text(text="🔄 *Spinning and rewriting your article using Gemini... Please wait.*", parse_mode="Markdown")
    
    # Prompt Engineering for the Spinner
    system_prompt = (
    ...
    try:
        response = ai_client.chat.completions.create(
            model="gemini-2.5-flash",  # Fast, highly performant model
            messages=[
        ...
        await query.message.reply_text("❌ Sorry, something went wrong while spinning your article with Gemini. Please try again.")
...
async def lifespan(app: FastAPI):
    ...
    # Set webhook pointing to Render's public application URL
...
async def health_check():
    """Keeps the Render instance happy and functional."""
    return {"status": "healthy", "bot": "AISEO Gemini Rewriter ready"}
